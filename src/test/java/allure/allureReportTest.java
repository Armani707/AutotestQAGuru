...
public class allureReportTest {
    ...
    далее в поле Task добавить наименование таски для запуска.
     ...
     Параметризованные сборки в Jenkins. Чтобы создать параметризованную сборку, нужно в
    настройках сборки нажать на кнопку "Это параметризованная сборка", дальше появится
    кнопка "Добавить параметр", нажимаешь на нее и добавляешь например Выборочный (чоис)
    параметр. Главное потом в поле Tasks добавить наш параметр в скобках {}. А перед этим
    написать clean, чтобы каждый раз запускался новый чистый тест.
    */
